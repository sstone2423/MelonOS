        ...
        cycle(): void {
            ...
            if (!_MemoryManager.inBounds(this.pc)) {
                ...
                if (this.isExecuting) {
                    ...
                    let value;
                    // Get the next opCode
                    const opCode = _Memory.readMemory(this.pc);
                    // Update console log with current execution
                    _Kernel.krnTrace('CPU cycle: executing: ' + opCode);

                    switch(opCode) {
                        case OpCode.LoadAccWithConst: // Load the accumulator with a constant
                        ...
                        case OpCode.LoadAccFromMem: /* Load the accumulator from memory
                        ...
                        case OpCode.StoreAccInMem: /* Store the accumulator in memory
                            Get the hex memory address by looking at next 2 values in memory
                            and swapping because of little-endian */
                            hexString = _Memory.readMemory(this.pc + 1);
                            fullHexString = _Memory.readMemory(this.pc + 2) + hexString;
                        ...
                        case OpCode.AddWithCarry: /* Add with carry: Adds contents of 
                        ...
                        case OpCode.LoadXWithConst: // Load the X register with a constant
                            // Change the value to hex
                            this.xReg = parseInt(_Memory.readMemory(this.pc + 1), 16);
                            this.pc += 2;
                            break;

                        case OpCode.LoadXFromMem: /* Load the X register from memory
                            Get the hex memory address by looking at next 2 values in memory
                            and swapping because of little-endian */
                            hexString = _Memory.readMemory(this.pc + 1);
                            fullHexString = _Memory.readMemory(this.pc + 2) + hexString;
                        ...
                        case OpCode.LoadYWithConst: // Load the Y register with a constant
                            // Change the value to hex
                            this.yReg = parseInt(_Memory.readMemory(this.pc + 1), 16);
                            this.pc += 2;
                            break;

                        case OpCode.LoadYFromMem:  /* Load the Y register from memory
                            Get the hex memory address by looking at next 2 values in memory
                            and swapping because of little-endian */
                            hexString = _Memory.readMemory(this.pc + 1);
                            fullHexString = _Memory.readMemory(this.pc + 2) + hexString;
                        ...
                        case OpCode.NoOp: // No operation.. Just increment
                        this.pc++;
                        break;

                        case OpCode.Break: // Break (system call)
                        _KernelInterruptQueue.enqueue(new Interrupt(PROCESS_EXIT_IRQ, true));
                        break;

                        case OpCode.CompareMemToX: /* Compare a byte in memory to the X reg: 
                            ...
                            const byte = _Memory.readMemory(address);
                        ...
                        case OpCode.BranchNBytes: // Branch n bytes if Z flag = 0
                            if (this.zFlag == 0) {
                                // Get the number of bytes to branch
                                const branch = parseInt(_Memory.readMemory(this.pc + 1), 16);
                                this.pc = _Memory.branchLoop(this.pc, branch,
                                    _MemoryManager.runningProcess.partition);
                            } else {
                                this.pc += 2;
                            }
                            break;

                        case OpCode.Increment: /* Increment the value of a byte
                            Get the hex memory address by looking at next 2 values in memory
                            and swapping because of little-endian */
                            hexString = _Memory.readMemory(this.pc + 1);
                            fullHexString = _Memory.readMemory(this.pc + 2) + hexString;
                        ...
                        case OpCode.SysCall: /* System call
                            ...
                            if (this.xReg === 1) {
                                ...
                                const hex = parseInt(original.toString(), 16);
                                // Get the character from the value
                                let chr = String.fromCharCode(hex);
                                while (original != "00"){
                                    const ascii = _Memory.readMemory(address);
                                    // Convert hex to decimal
                                    const dec = parseInt(ascii.toString(), 16);
                                ...
                                _KernelInterruptQueue.enqueue(
                                    new Interrupt(CONSOLE_WRITE_IRQ, printString));
                            }
                            this.pc++;
                            break;
