...
module TSOS {
    ...
    export class DeviceDriverDisk extends DeviceDriver {
        ...
        createFile(filename: String): number {
            const check = this.checkForExistingFile(filename);
            ...
            for (let sectorNum = 0; sectorNum < _Disk.totalSectors; sectorNum++) {
                for (let blockNum = 0; blockNum < _Disk.totalBlocks; blockNum++) {
                    ...
                    const tsbId = "0" + ":" + sectorNum + ":" + blockNum;
                    ...
                    if (dirBlock.availableBit == "0") {
                        ...
                        const dataBlockTSB = this.findFreeDataBlock();
                        if (dataBlockTSB != null) {
                            ...
                            const hexArray = Utils.stringToASCIItoHex(filename);
                        ...
                        // We ran through the data structure but there were no free blocks, 
                        // meaning no more space on disk
                        return DISK_IS_FULL;
                    }
                }
            }

            // We ran through the data structure but there were no free blocks, meaning 
         ...
         * @param filename 
         */
        checkForExistingFile(filename: String) {
            let check;
            const hexArray = Utils.stringToASCIItoHex(filename);
            for (let sectorNum = 0; sectorNum < _Disk.totalSectors; sectorNum++) {
                for (let blockNum = 0; blockNum < _Disk.totalBlocks; blockNum++) {
                    ...
                    const tsbId = "0" + ":" + sectorNum + ":" + blockNum;
                    const dirBlock = JSON.parse(sessionStorage.getItem(tsbId));
                    const matchingFileName = true;
                    check = {
                        "tsbId": tsbId,
        ...
        findFreeDataBlock() {
            ...
            for (let trackNum = 1; trackNum < _Disk.totalTracks; trackNum++) {
                for (let sectorNum = 0; sectorNum < _Disk.totalSectors; sectorNum++) {
                    for (let blockNum = 0; blockNum < _Disk.totalBlocks; blockNum++) {
                        const tsbId: string = trackNum + ":" + sectorNum + ":" + blockNum;
                        const dataBlock = JSON.parse(sessionStorage.getItem(tsbId));
                        // If the block is available, mark it as unavailable, and set its 
                        // tsb to the dirBlock pointer
         ...
         * @param pointer_tsb 
         */
        deleteData(pointer_tsb): void {
            // Block that belongs to the TSB
            const ptrBlock = JSON.parse(sessionStorage.getItem(pointer_tsb));
         ...
         * @param formatType 
         */
        format(formatType: number): boolean {
            ...
            if (formatType == QUICK_FORMAT) {
                for (let i = 0; i < _Disk.totalTracks * _Disk.totalSectors * _Disk.totalBlocks; i++) {
                    ...
                    const block = JSON.parse(sessionStorage.getItem(sessionStorage.key(i)));
            ...
            // Default to full format
                ...
                // For all values in session storage, set available bit to 0, 
                // pointer to 0,0,0, and fill data with 00s
                ...
                for (let j = 0; j < _Disk.totalTracks * _Disk.totalSectors * _Disk.totalBlocks; j++) {
                    // Get the JSON from the stored string
                    const block = JSON.parse(sessionStorage.getItem(sessionStorage.key(j)));
                    block.availableBit = "0";
                    block.pointer = "0:0:0";
                    block.data = zeroes;
            ...
            const size = _MemoryManager.residentQueue.getSize();
        ...
        listFiles(): Array<String> {
            ...
            for (let sectorNum = 0; sectorNum < _Disk.totalSectors; sectorNum++) {
                for (let blockNum = 0; blockNum < _Disk.totalBlocks; blockNum++) {
                    ...
                    // Don't look in blocks not in use
                    if (dirBlock.availableBit == "1") {
                        const size = this.getTsbSize(dirBlock.pointer);
                        const info = {
                            data: dirBlock.data,
                            size: size + " bytes"
                        };
            ...
            for (let i = 0; i < filenames.length; i++) {
                ...
                filenames[i]['year'] = parseInt(filenames[i]['data'][2]
                     + filenames[i]['data'][3], 16);
            }
        ...
        readFile(filename: String) {
            ...
            // If name is found
            if (check.matchingFileName) {
                const dirBlock = JSON.parse(sessionStorage.getItem(check.tsbId));
                // Perform a recursive read
                const tsb = dirBlock.pointer;
                const data = this.readData(tsb);
                let dataPtr = 0;
         ...
         * @param tsb 
         */
        readData(tsb: string): Array<string> {
         ...
         * @param data 
         */
        writeFile(filename: string, data: string): number {
            const check = this.checkForExistingFile(filename);
            // If name is found
            if (check.matchingFileName) {
                const dirBlock = JSON.parse(sessionStorage.getItem(check.tsbId));
                // Convert the text to a hex array, trimming off quotes
                const dataHexArray = Utils.stringToASCIItoHex(data.slice(1, -1));
                // Allocates enough free space for the file
                const freeSpace: boolean = this.allocateDiskSpace(dataHexArray, dirBlock.pointer);
                    if (!freeSpace) {
                        return DISK_IS_FULL;
                    }
                // We have enough allocated space. Get the first datablock, 
         ...
         * @param dataHexArray 
         */
        writeDataToFile(tsb: string, dataHexArray: Array<String>): void {
            let dataPtr = 0;
            ...
            for (let i = 0; i < dataHexArray.length; i++) {
                ...
                dataPtr++;
                // Check to see if we've reached the limit of what data the block can hold. 
                // If so, go to the next block.
         ...
         * @param tsb 
         */
        allocateDiskSpace(file: Array<String>, tsb: string): boolean {
            ...
            let dataBlock = JSON.parse(sessionStorage.getItem(dataBlockTSB));
            // If data block we're writing to is already pointing to something, 
            // we need to traverse it, making sure there is enough space to 
            // hold our new file. Continuously allocate new blocks
            while (stringLength > _Disk.dataSize) {
                ...
                if (dataBlock.pointer != "0:0:0" && dataBlock.availableBit == "1") {
                    ...
                    // We reached the end of the blocks that have already been allocated 
                    // for this file. Mark the starting block as in use
                    dataBlock.availableBit = "1";
                    // First, find out how many more datablocks we need
                    const numBlocks = Math.ceil(stringLength / _Disk.dataSize);
                    // Go find that number of free blocks
                    const freeBlocks = this.findFreeDataBlocks(numBlocks);
                    if (freeBlocks != null) {
                        // Once we get those n blocks, mark them as used, then set 
                        // their pointers accordingly. Set the current block's pointer 
                        // to the first block in the array, then recursively set pointers
         ...
         */
        findFreeDataBlocks(numBlocksNeeded: number) {
            const blocks = [];
         ...
         * @param opCodes 
         */
        writeSwap(filename: String, opCodes: Array<String>): number {
            ...
            if (check.matchingFileName) {
                // Allocates enough free space for the file
                const dirBlock = JSON.parse(sessionStorage.getItem(check.tsbId));
                const dataBlock = JSON.parse(sessionStorage.getItem(dirBlock.pointer));
                dataBlock.availableBit = "0";
                sessionStorage.setItem(dirBlock.pointer, JSON.stringify(dataBlock));
                // Check if there is enough space
                const freeSpace: boolean = this.allocateDiskSpace(opCodes, dirBlock.pointer);
                if (!freeSpace) {
                    return DISK_IS_FULL;
                }
                // We have enough allocated space. Get the first datablock, 
                // keep writing until no more string.
